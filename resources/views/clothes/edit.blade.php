...
@section('content')
    <div class="mb-6 mt-10 md:mt-0 pt-5">
        <h2 class="text-2xl font-bold text-gray-800">Edit Clothes</h2>
        <p class="text-gray-600">Update clothing item details.</p>
    </div>

    <!-- Form Section -->
    <div class="bg-white rounded-xl shadow-sm border border-pink-100 p-6">
        <form action="{{ route('clothes.update', $clothes->id) }}" method="POST"> <!-- Action untuk update -->
            @csrf <!-- CSRF Token -->
            ...
            <div class="mb-6">
                ...
                <select id="label" name="label"
                    ...
                    <option value="dewasa" {{ $clothes->label == 'dewasa' ? 'selected' : '' }}>Dewasa</option>
                    <option value="anak" {{ $clothes->label == 'anak' ? 'selected' : '' }}>Anak</option>
                </select>
            </div>
