...
@section('content')
    <div class="mb-6 mt-10 md:mt-0 pt-5">
        <h2 class="text-2xl font-bold text-gray-800">Edit Size</h2>
        <p class="text-gray-600">Update size details.</p>
    </div>

    <!-- Form Section -->
    <div class="bg-white rounded-xl shadow-sm border border-pink-100 p-6">
        <form action="{{ route('sizes.update', $size->id) }}" method="POST"> <!-- Action untuk update -->
            ...
            <div class="mb-6">
                ...
                <select id="label" name="label"
                    ...
                    <option value="dewasa" {{ $size->label == 'dewasa' ? 'selected' : '' }}>Dewasa</option>
                    <option value="anak" {{ $size->label == 'anak' ? 'selected' : '' }}>Anak</option>
            ...
            <div class="mb-6">
                ...
                <input type="checkbox" id="is_custom" name="is_custom" value="1"
                    ...
                    {{ $size->is_custom ? 'checked' : '' }}> <!-- Checkbox untuk is_custom -->
            </div>
